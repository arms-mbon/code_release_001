# This code will read an input file with aphiaIDs (at a minumum) and return a set of attributes for those taxa
...
# The results are returned to a copy of the input file with the extra information added
# This version is fine-tuned to the ARMS use-case for the first data submission
# version 2 of this code is a more general instance
...
infile = location + '\SpeciesList.csv'
outfile = location + '\SpeciesListAttributes.csv'
...
with open(outfile, 'w', newline='') as csvfileout:
    ...
    writer.writerow(["Species","AphiaID","Unitid","ObservatoryID","Species importance to society", "IUCN RedList Category","IUCN Criteria","IUCN Year Accessed","HELCOM RedList Category","AMBI ecological group","Environmental position"])
    i=0
    with open(infile, 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        next(reader)
        for row in reader:
            ...
            try:
                ...
                #print("exception:",i,"   ",e)
                taxadict[aphia_id] = ["None","None","None","None","None","None","None"]
            writer.writerow([cell1,aphia_id,cell3,cell4,taxadict[aphia_id][0],taxadict[aphia_id][1],taxadict[aphia_id][2],taxadict[aphia_id][3],taxadict[aphia_id][4],taxadict[aphia_id][5],taxadict[aphia_id][6]])
